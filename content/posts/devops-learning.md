...
CI(import 'prismjs/themes/prism.css')，持续集成。即开发人员在工作时不断地集成工作分支到主分支（一天内，频繁地将代码集成到主干）。这样不仅可以提高开发效率，还可以自动测试和构建工作，快速迭代的时候还可以及时地发现错误。同时，也防止分支大幅地偏离主干。因为如果不是经常集成的话，主干又在不断地更新，那么以后集成的难度也会增大，甚至难以集成。

### CD

CD(import 'prismjs/themes/prism.css')，持续交付。即频繁地将集成的产品交付给质量团队或者用户进行下一步的评审，通过了才会部署到生产阶段。
